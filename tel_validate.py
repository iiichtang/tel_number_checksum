...
HOTLINE_FIVE_DIGIT_DICT = {'12': 1, '95': 1}
...
def tel_validate(tel_string):
    ...
    if tel_without_area_code is not False:
        # 96 Hotline
        if tel_without_area_code[:2] == "96" and 4 < len(tel_without_area_code) < 8:
            # print("96 Hotline")
            return True
...
def region_check(tel_string):
    ...
    if not tel_string.startswith("+"):
        return False

    # Check if tel number contain any non int charactor
    if not str(tel_string[1:]).isdigit():
        return False

    # In America
    if tel_string.startswith("+1"):
        # print("+1 region")
        return True

    # In China
    if tel_string.startswith("+86"):
        local_tel_without_region = tel_string[3:]
    ...
    # In other country
    tel_first_three = tel_string[:4]
    tel_first_two = tel_string[:3]
